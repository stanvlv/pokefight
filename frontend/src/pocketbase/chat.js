...
const startingChat = await chatCollection.getList(1, 20, {
  sort: "-created",
  expand: "sender",
});
...
chatCollection.subscribe("*", (data) => {
  if (data.action === "create") {
    ...
    jotaiStore.set(chatArrayAtom, [
      data.record,
      ...jotaiStore.get(chatArrayAtom),
    ]);
  } else {
...
export async function writeToChat(message) {
  ...
  await chatCollection.create({ sender: me.id, message });
}
