...
jotaiStore.set(lobbyArray, initialLobby.map(item => ({id: item.id, 
    name: item.name, 
    status: item.status, 
...
function heartbeat(id) {
    return async () => {
        try {
            ...
            const res = await lobbyCollection.update(id, {ping: lastMeasuredPing}); // dont know if it works this way: YES IT FREAKIN DOES
        } catch (err) {
